...
function stageFile(projectId: string, stage: Stage, name: string): string {
  return path.join(stageDir(projectId, stage), name);
}

export function draftDslPath(projectId: string, stage: Stage): string {
  return stageFile(projectId, stage, FILE_NAMES.draftDsl);
}
export function docJsonPath(projectId: string, stage: Stage): string {
  return stageFile(projectId, stage, FILE_NAMES.docJson);
}
export function outHwpxPath(projectId: string, stage: Stage): string {
  return stageFile(projectId, stage, FILE_NAMES.outHwpx);
}

export async function readDraftDsl(projectId: string, stage: Stage): Promise<string | null> {
  const p = draftDslPath(projectId, stage);
  if (!existsSync(p)) return null;
  return readFile(p, "utf8");
}

export async function writeDraftDsl(projectId: string, stage: Stage, dsl: string): Promise<string> {
...
export async function readDocJson<T = unknown>(projectId: string, stage: Stage): Promise<T | null> {
  const p = docJsonPath(projectId, stage);
  if (!existsSync(p)) return null;
  return JSON.parse(await readFile(p, "utf8")) as T;
}

export async function writeDocJson(projectId: string, stage: Stage, doc: unknown): Promise<string> {
  ensureStageDir(projectId, stage);
  const p = docJsonPath(projectId, stage);
...
export async function readOutHwpx(projectId: string, stage: Stage): Promise<Buffer | null> {
  const p = outHwpxPath(projectId, stage);
  if (!existsSync(p)) return null;
  return readFile(p);
}

export async function writeOutHwpx(projectId: string, stage: Stage, bytes: Uint8Array): Promise<string> {
  ensureStageDir(projectId, stage);
  const p = outHwpxPath(projectId, stage);
...
export function appendRunLogLine(projectId: string, stage: Stage, runId: string, line: string): string {
...
export function readRunLogLines(projectId: string, stage: Stage, runId: string): string[] {
  const p = runLogPath(projectId, stage, runId);
  if (!existsSync(p)) return [];
  return readFileSync(p, "utf8").split("\n").filter((l) => l.trim().length > 0);
}

export function writeStageTextFile(projectId: string, stage: Stage, name: string, text: string): string {
  ensureStageDir(projectId, stage);
...
export function readStageTextFile(projectId: string, stage: Stage, name: string): string | null {
  const p = stageFile(projectId, stage, name);
  if (!existsSync(p)) return null;
