...
- 장은 "# 일 반 현 황"(간지), 절은 "## 설립목적"(번호 칩)으로 쓴다. Ⅰ·Ⅱ 와 1·2 는 자동으로 매겨지므로 번호를 직접 적지 않는다. 간지 줄은 앞뒤로 <pagebreak> 로 감싼다 — 앞의 쪽 나눔이 없으면 간지가 쪽 한가운데 찍힌다. 간지 뒤는 <pagebreak> · 빈 줄 · <pagebreak> 석 줄로 뒷면을 빈 쪽으로 남긴다.
